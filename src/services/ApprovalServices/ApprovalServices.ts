...
export async function getApprovalDetail(entityId: any, entityType: any) {
    return getData(`api/approval-requests?entityId=${entityId}&entityType=${entityType}`);
}  


// Phê duyệt báo cáo
export async function putApprovalReport(reportType: string, reportId: any, data: any) {
    return putData(`
      api/approval-requests/${reportType}/${reportId}/approving`,data
    )
};
